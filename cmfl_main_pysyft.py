...
def global_train(args, model, device, train_loaders, it, last_update):
    ...
    for batch_idx, (data, target) in enumerate(train_loaders):
        print(batch_idx,data.location,data.shape)
        import pdb; pdb.set_trace()  # breakpoint e1397a15 //
...
def main():
    ...
    worker_hooks = tuple([sy.VirtualWorker(hook,id=str(i)) for i in range(args.client_num)])
    worker1 = sy.VirtualWorker(hook,id="bob")
    worker2 = sy.VirtualWorker(hook,id='alice')
    ...
    train_loaders = sy.FederatedDataLoader(datasets.MNIST('../data',train=True,download=True,transform=transforms.Compose([
                                               transforms.ToTensor(),
                                               transforms.Normalize((0.1307,), (0.3081,))])).federate(worker_hooks),batch_size=args.batch_size,shuffle=True,**kwargs)

    # train_loaders = sy.FederatedDataLoader(datasets.MNIST('../data',train=True,download=True,transform=transforms.Compose([
    #                                            transforms.ToTensor(),
    #                                            transforms.Normalize((0.1307,), (0.3081,))])).federate((worker1,worker2)),batch_size=args.batch_size,shuffle=True,**kwargs)

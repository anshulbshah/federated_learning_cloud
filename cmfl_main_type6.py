...
def combine_updates(local_updates,relevances,topk):
    ...
    for indx in range(len(relevances)):
        if indx in where_relevant:
            deviation = {}
            deviations_list = []
            for k in local_updates[indx].keys():
                # if 'fc2' in k:

                deviation[k] = torch.sqrt((effective_gradients[k]  - local_updates[indx][k])**2 / num_relevant)
...
def client_train(args,ck,model,last_update,data_loader,device,threshold):
    ...
    # print(avg_sign)
    return rel,local_model_update,avg_sign,local_model


def global_train(args, model, device, train_loaders, it, last_update, previous_relevances,previous_local_updates,prev_local_models,prev_global_updates):
    """
    ...
    for ck in range(len(train_loaders)):
        if previous_relevances[ck]:
            rel,loc_up,avg_sign,local_model = client_train(args,ck,model,last_update,train_loaders[ck],device,threshold)
        else:            
        ...
        prev_local_models[ck] = local_model
...
def main():
    ...
    for it in tqdm(range(1, args.max_iterations)):
        (c_round_cmfl,c_round_after_sel),last_update,avg_sign,thresh,rel_it, prev_local_updates, prev_local_models, prev_global_updates = global_train(args, model, device, train_loaders, it,last_update, previous_relevances,prev_local_updates, prev_local_models, prev_global_updates)
